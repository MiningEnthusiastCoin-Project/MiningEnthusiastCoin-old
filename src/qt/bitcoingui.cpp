    ...
    platformStyle(_platformStyle)
{
    ...
    if(fCheckForUpdates && qtumVersionChecker->newVersionAvailable())
    {
        ...
        QString message(tr("New version of MiningEnthusiastCoin wallet is available on the MiningEnthusiastCoin source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a MiningEnthusiastCoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and miningenthusiastcoin: URIs)"));
    ...
    QRCTokenAction = new QAction(platformStyle->MultiStatesIcon(":/icons/qrctoken"), tr("&MRC Tokens"), this);
    QRCTokenAction->setStatusTip(tr("MRC Tokens (send, receive or add Tokens in list)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your MiningEnthusiastCoin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified MiningEnthusiastCoin addresses"));
    ...
    showBackupsAction = new QAction(platformStyle->MenuColorIcon(":/icons/openbackup"), tr("Show Automatic &Backups"), this);
    ...
    openAction->setStatusTip(tr("Open a miningenthusiastcoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible MiningEnthusiastCoin command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to MiningEnthusiastCoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void BitcoinGUI::updateHeadersSyncProgressLabel()
{
    ...
    int estHeadersLeft = (GetTime() - headersTipTime) / (Params().GetConsensus().nPowTargetSpacing / 2);
...
{
    QString strTitle = tr("MiningEnthusiastCoin"); // default title
