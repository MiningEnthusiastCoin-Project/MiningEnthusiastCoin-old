...
#if defined(NDEBUG)
# error "MiningEnthusiastCoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "MiningEnthusiastCoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s MNEC additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if (nHeight <= 525600)
        return 1000 * COIN;
    if (nHeight >= 525601 && nHeight <= 1051200)
        return 500 * COIN;
    if (nHeight >= 1051201 && nHeight <= 2102400)
        return 250 * COIN;
    if (nHeight >= 2102401 && nHeight <= 3504000)
        return 100 * COIN;
    if (nHeight >= 3504001 && nHeight <= 4380000)
        return 200 * COIN;
    if (nHeight >= 4380001 && nHeight <= 5606400)
        return 100 * COIN;
    return nSubsidy;
}
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    if (fCheckpointsEnabled) {
        ...
        if (pcheckpoint && nHeight < pcheckpoint->nHeight)
            return state.DoS(100, error("%s: forked chain older than last user-checkpoint (height %d)", __func__, nHeight), REJECT_CHECKPOINT, "bad-fork-prior-to-user-checkpoint");
    }
