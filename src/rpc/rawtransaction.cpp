...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx,    (numeric or string, required) The key is the MNEC address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
            ...
            "         \"amount\":x.xxx,                (numeric, optional) Value in HTML to send with the call, should be a valid amount, default 0\n"
...
UniValue decoderawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"VWTeUyCZsePENeFSMz7ydfY9HNevKiCChD\"   (string) miningenthusiastcoin address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
