...
const char * const BITCOIN_CONF_FILENAME = "miningenthusiastcoin.conf";
const char * const BITCOIN_PID_FILENAME = "miningenthusiastcoin.pid";
...
#else
    const char* pszModule = "miningenthusiastcoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MiningEnthusiastCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MiningEnthusiastCoin
    // Mac: ~/Library/Application Support/MiningEnthusiastCoin
    // Unix: ~/.miningenthusiastcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MiningEnthusiastCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MiningEnthusiastCoin";
#else
    // Unix
    return pathRet / ".miningenthusiastcoin";
#endif
#endif
}
